...
In the test run of _E. coli K12_, bcftools detected 299 SNPs and 20 indels. In 20 indels, 7 of them are labelled as mismatch but because of the different way of recording as described before. In this case, for _E. coli K12_, the rate of recall is 99.69%, and precision is same.

In the test run of _P. falciparum_, bcftools detected 300 SNPs with one false detection and one missing detection, and 20 indels, where 12 of them labeled as mismatch but can be explained same as above. Therefore, the rate of recall is 99.38%, and precision is same.
